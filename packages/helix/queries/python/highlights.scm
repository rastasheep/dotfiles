; Python syntax highlighting with docstring support
;
; Extends upstream Helix to capture docstrings as @string.documentation
; instead of generic @string, following nvim-treesitter convention:
; https://github.com/nvim-treesitter/nvim-treesitter/blob/master/queries/python/highlights.scm
;
; CHANGES FROM UPSTREAM:
; - Added: (expression_statement (string) @string.documentation)
;   Captures Python docstrings (per PEP 257) with semantic type @string.documentation
; - Pattern placed AFTER (string) @string to override via tree-sitter specificity
;
; This allows themes to style documentation strings separately from data strings.
...
; Regular strings (generic rule first)
(string) @string

; Docstrings as documentation (more specific rule AFTER generic, so it overrides)
